        ...
        ZStack {
            ...
            VStack(spacing: 16) {
                ...
                Button(editingWorkout == nil ? "Add Workout" : "Update Workout") {
                    ...
                    clearInputs()
                    workouts = DBHelper.shared.fetchAllWorkoutsORM()
                }
                .padding()
                ...
                HStack {
                    ...
                    Button("Apply Filter") {
                        if filterText.isEmpty {
                            ...
                            stats = DBHelper.shared.fetchStatsForExercise(exercise: filterText)
                        }
                    }
                    .padding(.horizontal)
                }
                ...
                List {
                    ForEach($workouts, id: \.workout_id) { workout in
                        VStack(alignment: .leading, spacing: 6) {
                            Text("Exercise: \(workout.wrappedValue.exercise)").bold()
                            Text("Weight: \(workout.wrappedValue.weight)")
                            Text("Reps: \(workout.wrappedValue.reps)")
                            Text("Sets: \(workout.wrappedValue.sets)")
                            Text("Notes: \(workout.wrappedValue.notes)")
                            HStack {
                                Spacer()
                                Button(action: {
                                    let unwrapped = workout.wrappedValue
                                    exercise = unwrapped.exercise
                                    weight = "\(unwrapped.weight)"
                                    reps = "\(unwrapped.reps)"
                                    sets = "\(unwrapped.sets)"
                                    notes = unwrapped.notes
                                    editingWorkout = unwrapped
                                }) {
                                ...
                                Button(action: {
                                    let unwrapped = workout.wrappedValue
                                    DBHelper.shared.delete(workout: unwrapped)
                                    workouts = DBHelper.shared.fetchAllWorkoutsORM()
                                    if editingWorkout?.workout_id == unwrapped.workout_id {
                                        editingWorkout = nil
                                        clearInputs()
                                    }
                                }) {
